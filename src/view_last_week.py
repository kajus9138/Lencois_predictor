...
def exibir():
    ...
    if len(df_fc_mon) > 0 and len(df_fc_mon) == len(df_med_mon):
        ...
        metricas_mon_text = f"MAE: {mae_mon:.2f} cm\nRMSE: {rmse_mon:.2f} cm\nErro Máx: {erro_max_mon:.2f} cm"
    else:
        metricas_mon_text = "Sem previsões anteriores\npara comparação"

    if len(df_fc_jus) > 0 and len(df_fc_jus) == len(df_med_jus):
        ...
        metricas_jus_text = f"MAE: {mae_jus:.2f} cm\nRMSE: {rmse_jus:.2f} cm\nErro Máx: {erro_max_jus:.2f} cm"
    else:
        metricas_jus_text = "Sem previsões anteriores\npara comparação"
