...
arima_mon = os.path.join(raiz, 'dados', 'modelos', 'sarimax_mon.pkl')
arima_jus = os.path.join(raiz, 'dados', 'modelos', 'sarimax_jus.pkl')
...
def insere_forecasts(modelo, estacao_id, day_1): 
    ...
    for ts_alvo, (orig_ts, nivel_previsto_cm) in zip(timestamps_range, preds.items()):
        ...
        #nivel_inf = float(intervalo[0]) Tentativa de ajuste com novo codigo abaixo
        nivel_inf = float(intervalo.iloc[0])
        #nivel_sup = float(intervalo[1]) Tentativa de ajuste com novo codigo abaixo
        nivel_sup = float(intervalo.iloc[1])
